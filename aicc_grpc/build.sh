...
docker buildx build --load ${PLATFORM_ARG} -t "${IMAGE_NAME}_aia:${VERSION}" --build-arg "IMAGE_NAME=${IMAGE_NAME}" "dist/${IMAGE_NAME}/${VERSION}/aia"

# adt용 빌드 이미지 생성
cp -r /root/go/src/grpc_adt dist/${IMAGE_NAME}/${VERSION}/adt/source
docker buildx build --load ${PLATFORM_ARG} -t "${IMAGE_NAME}_adt:${VERSION}" --build-arg "IMAGE_NAME=${IMAGE_NAME}" "dist/${IMAGE_NAME}/${VERSION}/adt"
